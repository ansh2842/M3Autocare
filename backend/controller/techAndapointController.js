...
exports.updateTechandDate = asyncHandler(async (req, res) => {
  ...
  try {
    let existingTechAndApp = await techAndAppModel.findOne({
    ...
    res
      .status(500)
      .json({ error: "Failed to create or update technician information" });
  }
});
...
exports.getDataByids = asyncHandler(async (req, res) => {
  ...
  try {
    const documents = await techAndAppModel.find({
      "technicians.technicianId": id,
    });
  ...
  try {
    ...
    if (result) {
      for (const doc of result) {
        const technicianIndex = doc.technicians.findIndex(
