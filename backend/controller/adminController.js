...
exports.datas = asyncHandler(async (req, res) => {
        ...
        try {
            ...
            const data = await adminModel.create({
                ...
                contact: '8089946865',
                location: 'Areekode',
                profileInformation:'Hi, I’m Anshad, I’m a full stack web developer and I have much experience in React js , MongoDB , NodeJS and express Js . I am also a freelancer.',
                facebook:'https://m.facebook.com/profile.php/?id=100053291650254&name=xhp_nt__fb__action__open_user',
...
exports.adminData = asyncHandler(async(req,res)=>{
    ...
    try{
          ...
          if(!Find){
           ...
           return res.status(400).json({invalid:true, message: "invalid details" });
           return;
          }
...
exports.getAdminDataForUpdate = asyncHandler(async(req,res) =>{
    ...
    const {id} = req.params;
    const {image,role,username,email,contact,location,profileInformation,facebook,twitter,instagram} = req.body
...
exports.changeForgotPassword = asyncHandler(async(req,res)=>{
    ...
    try{
        if(newPassword === confirmPassword){
            ...
            const forgotpasswordChange = await adminModel.findById(id)
            const passwordMatch =await bcrypt.compare(confirmPassword, forgotpasswordChange.password)
            forgotpasswordChange.password = confirmPassword
            
            forgotpasswordChange.password = confirmPassword
            let letToUpdates = await forgotpasswordChange.save()
            res.json(letToUpdates)
        }else{
