...
function products() {
  ...
  const getProduct = async (filterCategory) => {
    ...
    await axios
      .get(`http://localhost:8000/user/getFIlterProducts?category=${filterCategory}`)
      .then((res) => {
        setProduct(res.data);
      })
      .catch((err) => {
  ...
  const getCategory = async () => {
    await axios
      .get("http://localhost:8000/user/getCategory")
      .then((res) => {
              ...
              <div>
                <select style={{ width: "22%" }} onChange={HandleChange} className={styles.selects}>
                  ...
                  {Categorys.map((items) => (
                    <option
                      key={items.id}
                      onClick={() => handleFilter(items._id)}
                      value={items._id}
                    >
                      {items.Title}
                    </option>
        ...
        >
          ...
          <DialogContent>
            ...
            <DialogContentText>
              ...
              <span className={styles.deatils}>{products.name}</span>
            </DialogContentText>
            <DialogContentText>
              <span className={styles.dialogpop}>Brand:</span>{" "}
              <span className={styles.deatils1}>{products.brand}</span>
            </DialogContentText>
            <DialogContentText>
              <span className={styles.dialogpop}>Type:</span>{" "}
              <span className={styles.deatils1}>{products.subcategory}</span>
            </DialogContentText>
            <DialogContentText>
              <span className={styles.dialogpop}>Mrp:</span>{" "}
              <span className={styles.deatils2}>₹{products.mrp}</span>
            </DialogContentText>
            <DialogContentText>
              <span className={styles.dialogpop}>Price:</span>{" "}
              <span className={styles.deatils1}>₹{products.price}</span>
            </DialogContentText>

            <div className={styles.textflex}>
              <DialogContentText>
                <span className={styles.dialogpop}>About this item:</span>
                <br />
                <span
      ...
      <div>
        ...
        >
          ...
          <DialogContent>
            ...
            <DialogContentText>
              <span className={styles.dialogpop}>Name:</span>{" "}
              <span className={styles.deatils}>{getById.name}</span>
            </DialogContentText>
            <DialogContentText>
              <span className={styles.dialogpop}>Brand:</span>{" "}
              <span className={styles.deatils1}>{getById.brand}</span>
            </DialogContentText>
            <DialogContentText>
              <span className={styles.dialogpop}>Type:</span>{" "}
              <span className={styles.deatils1}>{getById.subcategory}</span>
            </DialogContentText>
            <DialogContentText>
              <span className={styles.dialogpop}>Mrp:</span>{" "}
              <span className={styles.deatils2}>₹{getById.mrp}</span>
            </DialogContentText>
            <DialogContentText>
              <span className={styles.dialogpop}>Price:</span>{" "}
              <span className={styles.deatils1}>₹{getById.price}</span>
            </DialogContentText>
            ...
            <div className={styles.textflex}>
              <DialogContentText>
                ...
                <br />
                <span
                  dangerouslySetInnerHTML={{ __html: getById.description }}
                  className={styles.deatils1}
                ></span>
              </DialogContentText>
            </div>
