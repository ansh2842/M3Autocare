...
function home() {
  ...
  await axios.get('http://localhost:8000/user/getProducts')
  .then(res =>{
    setProduct(res.data)
  }).catch(err=>{
              ...
              <div className={styles.row}>
                <div className="col-md-7 ">
                  <div className={`${styles.detail_box} ${styles.detaailbox}`}>
                    <h1  className={styles.cartext}>
                      {item.title}
                    </h1>
                  
                    <p className={styles.cartext5}>
                     {item.description}
                    </p>
           ...
           <div className="col-md-6 " key={item.id}>
             <div className={styles.box}>
               ...
               <div className={styles.detail_box}>
                 <h5 style={{fontFamily:'Onest, sans-serif'}}>
                  {item.name}
                 </h5>
                 <p  style={{fontFamily:'Onest, sans-serif'}}>
                 {item.description.slice(0, 100)}...
                 </p>
                 <Link  style={{fontSize:"17px",fontFamily:'Onest, sans-serif'}} to={`/user-service/${item._id}`}>
                   Read More
                </Link>
               </div>
             </div>
      ...
      <div className={styles.service_container}>
        <div className="container ">
          <div className={styles.heading_container}>
            ...
            <p style={{fontFamily:'Onest, sans-serif'}}>Our Technicians are build for cars...</p>
          </div>
          ...
          <OwlCarousel
            ...
            items={carouselItems}
          >
            {dataTech && dataTech.map((item) => (
                 <Card style={{ margin: '0 auto',marginTop:'30px',backgroundColor:' rgb(242, 242, 242)',border:'none' }} key={item.id}>
               <Card.Img variant="top" src={`http://localhost:8000/${item.image}`} />
               <Card.Body>
                 <Card.Title style={{color:'black',fontSize:'19px',textTransform:'uppercase',fontFamily:'Onest, sans-serif',fontWeight:"bold"}}>{item.name}</Card.Title>
                 <Card.Text style={{color:'black',fontSize:'14px',fontFamily:'Onest, sans-serif'}}>
  ...
  <div className='container'>
    <div className="row">
      {getDataAbout.map((item) => (
        <div className="col-md-6" key={item.id}>
          <div className={styles.detail_box}>
            <p style={{fontFamily:'Onest, sans-serif'}} dangerouslySetInnerHTML={{ __html: item.about }}></p>
            <Link style={{fontSize:"17px",fontFamily:'Onest, sans-serif'}} to='/user-about'>
              Read More
            </Link>
          </div>
        </div>
    ...
    <div className="container">
      <div className="row">
        <div className="col-md-6">
          ...
          <p style={{fontFamily:'Onest, sans-serif'}}>
            You can track your services by just enter the appointment id and your appointment id will be send to your email after you take an appointment.
          </p>
          <form onSubmit={(e)=> e.preventDefault()}>
            <input id='value' onChange={(e)=>setTrackData(e.target.value)} type="text" style={{fontFamily:'Onest, sans-serif',fontSize:'14px'}} placeholder="Enter Your Appointment Id" />
            {bthhide && (

            <button onClick={()=>handleTrack(trackData)} style={{fontFamily:'Onest, sans-serif',cursor:'pointer'}}>
              Track
            </button>
            )}
            {spinners && (
            ...
            {invalid &&(
              <p style={{fontFamily:'Onest, sans-serif',color:"red",fontSize:'14px'}}>{invalid}</p>
            )}
            <p style={{fontFamily:'Onest, sans-serif',color:"red",fontSize:'14px'}}>{trackerr}</p>
        </div>
      </div>
    </div>
...
<div className="shadow p-3 mb-5 bg-body rounded">
 <FontAwesomeIcon style={{color:"black",float:'right',cursor:'pointer'}} onClick={handleClose}  icon={faClose}/>
<div className="table-responsive table-borderless shadow-2-strong rounded-4">
    ...
    <p style={{fontFamily:'Onest, sans-serif',color:'black',fontSize:"18px"}} className="text-center">Here is your appointment details</p>
  <table style={{marginTop:'2rem'}} className="table">
    ...
    <tbody className="table-body">
     ...
     <td style={{ fontFamily: 'Onest, sans-serif' ,fontSize:'14px'}}>{getTrackDown.Appointment_id}</td>
     <td style={{ fontFamily: 'Onest, sans-serif', textTransform: 'capitalize',fontSize:'14px' }}>{getTrackDown.carName}</td>
     <td style={{ fontFamily: 'Onest, sans-serif', textTransform: 'uppercase',fontSize:'14px' }}>{getTrackDown.CarNumber}</td>
     <td style={{ fontFamily: 'Onest, sans-serif', textTransform: 'capitalize',fontSize:'14px' }}>{getTrackDown.location}</td>
     <td style={{ fontFamily: 'Onest, sans-serif', textTransform: 'capitalize' ,fontSize:'14px'}}>{getTrackDown.service}</td>
     <td style={{ fontFamily: 'Onest, sans-serif' }}>
     {getTrackDown.status === 'Received' &&(

<span className="badge badge-success"  style={{ fontFamily: 'Onest, sans-serif' ,fontSize:'14px',position:'relative',bottom:'5px',backgroundColor:'#0a97b0'}}>{getTrackDown.status}</span>
)}
{getTrackDown.status === 'Seeking Issue' &&(

<span className="badge badge-success"  style={{ fontFamily: 'Onest, sans-serif' ,fontSize:'14px',position:'relative',bottom:'5px',backgroundColor:'orange'}}>{getTrackDown.status}</span>
)}
{getTrackDown.status === 'Team on work' &&(

<span className="badge badge-success"  style={{ fontFamily: 'Onest, sans-serif' ,fontSize:'14px',position:'relative',bottom:'5px',backgroundColor:'brown'}}>{getTrackDown.status}</span>
)}
{getTrackDown.status === 'On progress' &&(

<span className="badge badge-success"  style={{ fontFamily: 'Onest, sans-serif' ,fontSize:'14px',position:'relative',bottom:'5px',backgroundColor:'purple'}}>{getTrackDown.status}</span>
)}
{getTrackDown.status === 'Checking all works' &&(

<span className="badge badge-success"  style={{ fontFamily: 'Onest, sans-serif' ,fontSize:'14px',position:'relative',bottom:'5px',backgroundColor:'cyan'}}>{getTrackDown.status}</span>
)}
{getTrackDown.status === 'we are on last' &&(

<span className="badge badge-success"  style={{ fontFamily: 'Onest, sans-serif' ,fontSize:'14px',position:'relative',bottom:'5px',backgroundColor:'pink'}}>{getTrackDown.status}</span>
)}
{getTrackDown.status === 'Work Done' &&(

<span className="badge badge-success"  style={{ fontFamily: 'Onest, sans-serif' ,fontSize:'14px',position:'relative',bottom:'5px'}}>{getTrackDown.status}</span>
)}
     </td>
     <td style={{ fontFamily: 'Onest, sans-serif' ,fontSize:'14px' }}>₹{getTrackDown.price}</td>
     <td style={{ fontFamily: 'Onest, sans-serif' ,fontSize:'14px' }}>{getTrackDown.appointment_date}</td>
  <button style={{color:'#0a97b0'}} onClick={()=>handleClickOpens1(getTrackDown._id)} className={styles.viewbtn}>View</button>
        ...
        <div className="col-lg-4 col-md-5 offset-md-1">
          <div className={`${styles.form_container} contact-form`}>
            ...
            <p style={{fontFamily:'Onest, sans-serif',color:"green",fontSize:'14px'}}>{success}</p>
              ...
              required
            />
                {validationError && <div style={{fontFamily:'Onest, sans-serif',color:"red",fontSize:'14px'}} className="error-message">{validationError}</div>}
        ...
        <div style={{display:'flex',gap:'50px'}}>
          ...
          <DialogContentText className={styles.dialogpop} >
           ...
           <span  className={styles.dialogpop}>Email:</span><span className={styles.deatils1}> {appdata.email}</span>
            ...
            <DialogContentText >
           <span  className={styles.dialogpop}>Location:</span><span style={{textTransform:'capitalize'}} className={styles.deatils1}> {appdata.location}</span>
            </DialogContentText>
            <DialogContentText >
