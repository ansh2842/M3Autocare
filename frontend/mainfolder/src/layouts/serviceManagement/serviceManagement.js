...
<td><img  style={{ width: '40px' }} src={`http://localhost:8000/${items.image[0]}` } alt=''></img></td>
<td>{items.name}</td>
<td>{items.description}</td>
<td style={{textDecoration:'line-through'}}>{items.mrp}</td>
<td>{items.price}</td>
<td><Link to={`/ServiceEdit/${items._id}`}><SoftButton variant="text" color="info"  fontWeight="medium">Edit</SoftButton></Link></td>
    ...
    <div>
      <Dialog TransitionComponent={Zoom} transitionDuration={400} open={open} onClose={handleClose}>
        ...
        <DialogContent>
          ...
          <SoftBox mb={2}>
              <input multiple onChange={(e) => setImag(e.target.files)}  accept="jpeg,jpg,png" type="file" />
          </SoftBox>
          <p style={{ color: "red", fontSize: "11px" }}>{imgError}</p>
          <SoftBox mb={2}>
              <SoftInput onChange={(e) => setName(e.target.value)}   type="text" placeholder="Service Name"/>
            </SoftBox>
            <p style={{ color: "red", fontSize: "11px" }}>{nameError}</p>
         
            <SoftBox mb={2}>
              <textarea style={{width:'100%',borderRadius:'7px',fontSize:'14px',paddingLeft:'4px',outline:'none'}} onChange={(e) => setDescription(e.target.value)}   placeholder="Description" type='text'  />
            </SoftBox>
