...
axios.post('http://localhost:8000/admin/subcategory',formData,{
  headers: {
     ...
     <SoftButton onClick={handleClickOpen}  variant="gradient" color="dark" >
      ...
      <tbody>
        {getData.map((item) =>(
            ...
            <td>{item.Title}</td>
            <td><Link to={`/SubCategoryedit/${item._id}`}><SoftButton variant="text" color="info"  fontWeight="medium">Edit</SoftButton></Link></td>
      ...
      <Dialog TransitionComponent={Zoom} transitionDuration={400} open={open} onClose={handleClose}>
        ...
        <DialogContent>
          ...
          <SoftBox mb={2}>
            ...
            <SoftBox mb={2}>
            <select onChange={(e) => setBrandName({ _id: e.target.value })} style={{ fontSize: "14px" }}>
