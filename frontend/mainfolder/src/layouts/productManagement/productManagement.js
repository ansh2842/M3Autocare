...
const categoryManagement = () => {
  ...
  const currentAppointments = product.slice(indexOfFirstAppointment, indexOfLastAppointment);
  ...
  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(product.length / appointmentsPerPage); i++) {
    pageNumbers.push(i);
  }
  ...
  const getProduct = async () => {
    ...
    await axios
      .get("http://localhost:8000/admin/getProducts")
      .then((res) => {
        setProduct(res.data);
      })
      .catch((err) => {
  ...
  const getsubCategoryData = async () => {
    try {
      await axios.get("http://localhost:8000/admin/getsubCategory").then((res) => {
  ...
  const getCategorys = async () => {
    ...
    await axios
      .get("http://localhost:8000/admin/getBrand")
      .then((res) => {
        setBrand(res.data);
      })
      .catch((err) => {
...
<tr style={{fontSize:'14px'}} key={items.id}>
          ...
          {currentAppointments.map((items, index) => (
          ...
          <DialogContent>
            ...
            <SoftBox mb={2}>
              <select
                ...
                style={{ fontSize: "14px", borderColor: !selectCategory ? "red" : "grey" }}
              >
                <option value="select" disabled selected>
                  Select Sub Category
                </option>
