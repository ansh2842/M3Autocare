...
const brandManagement = () => {
  ...
  const handleAdd = async () => {
    ...
    await axios
      .post("http://localhost:8000/admin/brandpost", formData, {
        headers: {
    ...
    <DashboardLayout>
        ...
        <tbody>
          {currentAppointments.map((items, index) => (
            <tr style={{ fontSize: "14px" }} key={index}>
              ...
              <td>
                <img style={{ width: "40px" }} src={`http://localhost:8000/${items.image}`}></img>
              </td>
              <td>{items.title}</td>

              <td>
                <Link to={`/BrandEdit/${items._id}`}>
      ...
      <div>
        ...
        >
          ...
          <DialogContent>
            <DialogContentText>Brand details</DialogContentText>
            <SoftBox mb={2}>
              <SoftInput
